...
1. **What is a "product" in a monorepo?** *Default (revised after M0/M2):* discover units by **convention scan** of `libs/*`, `apps/*`, `packages/*`, not by the root `workspaces` glob (it was stale on the real repo). The root is always a product. Attribution terminates at the nearest **local/workspace node in the lockfile**, so on real npm-workspaces repos exposure lands on the actual workspace (proven on the fixture). On SWWC v2's Nx setup deps sit at the root, so exposure lands there until the Nx project graph lands in **M3** — Bojji says which case it's in rather than guessing.

...
> [!OK] **M1 — shipped (2026-07-20).** `bojji expose &lt;CVE|GHSA&gt;` now resolves the affected npm package(s) + ranges from OSV, no manual flags. Validated end-to-end on the real SWWC v2 lockfile: **CVE-2021-44906** → `minimist@1.2.5` via `commitizen`; **CVE-2024-37890** → `ws@8.13.0` via `puppeteer` (four disjoint SEMVER intervals across major lines, matcher correctly flagged the 8.x copy and excluded the fixed 7.5.10 copies); **CVE-2023-26136** → correctly *not exposed* (repo has the patched tough-cookie 4.1.4). GHSA ids, `--json`, and the `--package/--range` offline override all work.

> [!NOTE] **OSV finding baked into M1:** a CVE record on OSV is often ecosystem-agnostic (`package: null`); the npm ranges live in its **GHSA alias**. M1 follows aliases automatically, so a bare CVE id resolves even when its own record has no npm data. (The plan's example, CVE-2023-45857/axios, happens to be *not exposed* here — the repo ships patched axios 1.13.5 — so the mechanism was proven on minimist and ws instead.)
...
> [!OK] **M2 — shipped (2026-07-20).** Ownership is derived live from CODEOWNERS at read time (`codeowners.ts` + `ownership.ts`), never stored. Each exposed product shows its owner, the matching CODEOWNERS pattern, an "as of" stamp, and a confidence: **teams → high**, **individuals → medium with a "prefer a team/role" flag** (the P3 "never a person" rule, enforced live), **no rule → an honest "set one to route this"** rather than a guessed owner. Attribution now lands on the real **npm-workspace unit**, not always the root. Demonstrated on a workspaces fixture (`fixtures/demo-monorepo`): `@acme/web` → `@acme/frontend-team`, `@acme/api` → `@acme/backend-team`. On SWWC v2 the exposed root correctly reports "no CODEOWNERS rule" (its CODEOWNERS only scopes `/apps/storybook/`).
...
> [!OK] **M3 — shipped (2026-07-20).** Per-library attribution on the real SWWC v2 repo via the **Nx project graph** (`nx.ts` reads Nx's cached `project-graph.json`; `--nx-graph`/`--no-nx` override). A root-level vuln is routed to the shipped libraries that actually import it, or flagged as **root/dev tooling** when none do, with an exposed / not-exposed breakdown across all 12 units. Proven on **CVE-2021-3803** (nth-check): one copy is dev tooling (via rehype), the other lands in the shipped **`swwc-core-styles`** (via cssnano → svgo → css-select), each with its full proving path. This is the "which shipped product, and who to notify" answer GitLab's flat row cannot give.
...
1. **What is a "product" in a monorepo?** *Resolved (M0 → M3):* discover units by **convention scan** of `libs/*`, `apps/*`, `packages/*`, not by the root `workspaces` glob (it was stale on the real repo). npm-workspaces repos attribute to the nearest **local/workspace node in the lockfile**; Nx repos (deps at the root) attribute via the **Nx project graph** to the shipped library that imports the vuln, or flag it as root/dev tooling. Both proven on real data. Bojji states which model it used rather than guessing.
...
3. **CVE → version-range matching.** *Resolved in M1:* OSV is the source of ranges; a bare CVE resolves by **following its GHSA alias** (the CVE record is often ecosystem-agnostic). SEMVER intervals are matched with `semver`; enumerated `versions[]` are matched exactly as a fallback. Validated on multi-interval advisories (ws across four major lines). Never hand-roll comparison.
